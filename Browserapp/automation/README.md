...
RPA 执行链路：`rpa_run_steps` / `rpa_run_plan` 支持 `wait:false` 立即返回 task_id，再用 `rpa_task_result` 轮询（返回值含 `variables` / `exports` / `remarks`）；同步等待最长 10 分钟；`rpa_task_delete` 可清理单条历史任务。
