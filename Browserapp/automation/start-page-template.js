...
<body>
  <main class="shell">
    ...
    <section class="band">
      <div class="section-head"><h2>IP 健康评分</h2><span class="section-note">IPPure 风险系数 + 本地透明规则；不是官方健康分</span></div>
    ...
    <section class="band">
      <div class="section-head"><h2>访问能力</h2><span class="section-note">出口 IP 地区 + 站点侧访问身份 + best-effort 解锁判断（非账号可用性保证）</span></div>
    ...
    <footer class="footer">本页的健康结果结合 IPPure 公开风险系数与本地透明规则，不等同于 IPPure 官方健康分，也不保证任何站点的账号可用性。代理、托管和移动网络标签来自出口查询服务；Canvas、WebGL 暴露代表可被读取的指纹表面，不等同于身份泄露。</footer>
  </main>
  ...
  (function(){
    ...
    function unlockLabel(item){var u=String(item&&item.unlock||'');if(u==='unlocked')return '已解锁';if(u==='blocked')return '未解锁/拦截';if(u==='reachable')return '可连接';if(u==='unreachable')return '不可达';return item&&item.ok?'可连接':'失败'}
    ...
    function checkReachability(){var map={google:'reach-google',youtube:'reach-youtube',tiktok:'reach-tiktok',x:'reach-x',chatgpt:'reach-chatgpt',wikipedia:'reach-wikipedia',facebook:'reach-facebook',instagram:'reach-instagram',reddit:'reach-reddit'};Object.keys(map).forEach(function(id){setCheck(map[id],'na','检测中','正在检测连通性、访问 IP 地区与解锁信号。')});api('/api/reachability').then(function(result){var data=result.data||{};Object.keys(map).forEach(function(id){var item=data[id]||{};var host=String(item.url||id).replace('https://','').replace('http://','');var line=formatAccessLine(item);var note=item.unlock==='unlocked'?'best-effort 解锁信号通过':item.unlock==='blocked'?'站点返回拦截/不可用信号':item.unlock==='reachable'?'连通正常，无独立解锁信号':item.error?String(item.error):'';if(item.ok||item.unlock==='unlocked'||item.unlock==='reachable'){setCheck(map[id],unlockState(item),unlockLabel(item),host+' · '+line+(note?'。'+note+'。':'。'))}else{setCheck(map[id],unlockState(item),unlockLabel(item),host+' 检测失败：'+(item.error||('HTTP '+(item.status||0)))+(line?'；'+line:'')+'。')}})}).catch(function(error){Object.keys(map).forEach(function(id){setCheck(map[id],'warn','失败','本地连通性检测失败：'+String(error&&error.message||error))})})}
