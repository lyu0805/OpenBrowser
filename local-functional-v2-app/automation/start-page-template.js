...
<body>
  ...
  (function(){
    ...
    function applyNetwork(net){if(!net)return;currentNetwork=net;var ip=net.ip||net.query||'';text('ip-ip',ip||'未检测');var ipEl=byId('ip-ip');if(ipEl)ipEl.classList.toggle('small',ip.length>22);text('country',net.country||net.countryCode);text('region',net.region||net.regionName);text('city',net.city);text('timezone',net.timezone);text('network-type',networkType(net));text('net-asn',net.asn);text('net-isp',net.isp);text('net-org',net.organization||net.org);text('net-location',[net.latitude,net.longitude].filter(function(v){return v!=null}).join(', ')||net.zip);text('asn-line',[net.asn,net.asName||net.isp].filter(Boolean).join(' · '));text('checked-at',net.checkedAt?'检测时间 '+new Date(net.checkedAt).toLocaleString():'出口数据已更新');var directMode=BOOT.networkMode==='direct'||String(net.protocol||'').toLowerCase()==='direct'||String(BOOT.proxyProtocol||'').toLowerCase()==='direct';text('network-mode',directMode?'直连模式':String(BOOT.proxyProtocol||net.protocol||'代理').toUpperCase()+' 代理');var error=byId('ip-error');if(error){error.textContent='';error.classList.remove('show')}evaluateConsistency();try{document.title=(ip?ip+' · ':'')+BOOT.title}catch(_){}}
    function showNetworkError(error){var message=String(error&&error.message||error||'未知错误');text('ip-ip','检测失败');var el=byId('ip-ip');if(el)el.classList.add('small');var detail=byId('ip-error');if(detail){detail.textContent=message;detail.classList.add('show')}addFinding('network','bad',4);toast('出口检测失败：'+message)}
    function refreshNetwork(){if(!pid){toast('无环境 ID');return Promise.resolve()}var button=byId('btn-refresh');if(button)button.disabled=true;return api('/api/network?pid='+encodeURIComponent(pid)+'&refresh=1').then(function(result){applyNetwork(result.data);toast('检测已更新')}).catch(showNetworkError).finally(function(){if(button)button.disabled=false})}
    ...
    byId('btn-refresh').addEventListener('click',refreshNetwork);
    collectFingerprint();collectWebrtc();checkReachability();renderScore();
    if(currentNetwork&&currentNetwork.ip)applyNetwork(currentNetwork);else refreshNetwork();
  })();
  </script>
</body>
</html>`;
