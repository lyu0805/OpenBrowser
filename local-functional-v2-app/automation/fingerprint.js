 ...
 *
 * Research mapping (AdsPower sunBrowserParams → standard Chrome/CDP layer):
 *  CanvasMark / WebGLMark / AudioFp / ClientRectFp  → seeded JS noise
 *  HardwareConcurrency / DeviceMemory              → navigator spoof
 *  TimeZone / Geoposition                          → Emulation CDP
 *  WebRTCAddress / DisableWebRTC                   → flags + RTCPeerConnection patch
 *  UserAgent / ClientHints / Langs                 → UA + languages spoof
 *  ScreenSize                                      → screen spoof + window-size
 *
 * Kernel-level SunBrowser patches cannot be replicated; this is functional isolation
 * via isolated user-data-dir + CDP Emulation + document-start injection.
 */
...
const WEBGL_PRESETS = {
  ...
  linux: [
    ...
    { vendor: 'Google Inc. (AMD)', renderer: 'ANGLE (AMD, AMD Radeon RX 580 Series (RADV POLARIS10), OpenGL 4.6)' },
    { vendor: 'Google Inc. (NVIDIA)', renderer: 'ANGLE (NVIDIA, NVIDIA GeForce GTX 1660 SUPER/PCIe/SSE2, OpenGL 4.6)' },
  ],
};
...
function buildFingerprint(profile = {}) {
  ...
  // AdsPower-like noise marks (numeric seeds)
  const canvasId = (u32(seed, 28) % 20000) - 10000;
  const webglId = (u32(seed, 32) % 20000) - 10000;
  const audioId = (u32(seed, 36) % 20000) - 10000;
  const clientRectsId = (u32(seed, 40) % 20000) - 10000;
  ...
  // --- User-Agent + Client Hints (Ads-style) ---
...
function buildInjectionScript(fp) {
  ...
  // UA + Client Hints (userAgentData) injected first — Ads parity layer
...
function chromeArgsForFingerprint(fp, profile = {}) {
  ...
  // Ads setUserAgent: --user-agent=...
  if (fp.userAgent) args.push(`--user-agent=${fp.userAgent}`);
  // Ads setJA3: PermuteTLSExtensions by Chrome major from UA
  if (fp.uaProfile) {
...
async function applyFingerprintToTab(cdpCall, webSocketDebuggerUrl, fp, profile = {}) {
  ...
  // Ads: Network/Emulation.setUserAgentOverride + UserAgentMetadata (Client Hints)
